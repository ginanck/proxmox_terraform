...
resource "null_resource" "disk_resize" {
  ...
  triggers = {
    ...
    disk_size = local.harbor_config.disk.size
  }

  connection {
    type        = "ssh"
    host        = split("/", local.harbor_config.initialization.ip_config.ipv4.address)[0]
    user        = local.harbor_config.initialization.user_account.username
